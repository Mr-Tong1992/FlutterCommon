...
class NioPainter extends NioBasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    double secondOffset = index == widget.count - 1
        ? radius
        : radius + ((index + 1) * (size + space));
    _paint.color = Color.fromRGBO(0, 0, 0, 0.5);
    _paint.strokeWidth = 5;
    canvas.drawLine( new Offset(secondOffset - 8, radius), new Offset(secondOffset + 8, radius), _paint);
  }
}
...
//        new Offset(radius + (page * (size + space)), radius), radius, _paint);
    ...
    //    canvas.drawLine(new Offset((page * (size + space)) - 4, radius), new Offset((page * (size + space)) + 8, radius), _paint);
    Rect rect = Rect.fromCenter(width: 12, height: 5, center: Offset(page * (size + space) + 3, radius));
    RRect rRect = RRect.fromRectAndRadius(rect, Radius.circular(2.5));
